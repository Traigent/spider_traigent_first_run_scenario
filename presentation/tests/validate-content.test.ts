...
describe("presentation content validation", () => {
  it("accepts the canonical contract-only deck and public catalog", () => {
    ...
    expect(validated.slides).toHaveLength(21);
  ...
  it("keeps coverage targets distinct from the one published scenario", () => {
    const matrix = presentation.slides
      .filter((slide) => slide.id === "spider-presets-matrix")
    ...
    expect(presentation.catalog[0]?.slug).toBe("spider-text-to-sql-benchmark");
  });
  ...
  it("pins every published-guide claim to the full reviewed revision", () => {
    ...
    expect(
      guideSlides.every(
        (slide) =>
          slide.sourceRevision === "75d338c31c97643c6a6d28a6aeef582d7b938db8",
      ),
    ).toBe(true);
  });
  ...
  it("keeps the four public evidence labels consistent across documentation", () => {
    const vocabularyDocuments = ["presentation/README.md"].map((relativePath) =>
    ...
    const runEvidenceDocuments = ["presentation/README.md"].map(
      (relativePath) =>
        readFileSync(path.join(repositoryRoot, relativePath), "utf8"),
    );
