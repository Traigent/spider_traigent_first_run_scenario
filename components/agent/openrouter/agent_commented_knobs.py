...
One model by default and one prompt. The settings it is tuned over are listed beside the
model roster below, and the configuration this agent is handed carries the values a run
chooses for them.
...
# The roster the agent calls from. The first entry is the default; the settings below say
# what else a run can choose.
MODELS = ("openrouter/qwen/qwen3-coder",)
MODEL = MODELS[0]
# Settings this agent is tuned over:
#   model: openrouter/qwen/qwen3-coder, openrouter/openai/gpt-oss-120b,
