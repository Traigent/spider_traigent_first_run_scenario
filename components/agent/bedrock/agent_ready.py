...
The models here are served by Amazon Bedrock. Bedrock signs with AWS credentials rather than an API key, and each model
has to be enabled in the account and region being used.
...
# Bedrock signs requests with AWS SigV4, which LiteLLM does through boto3 -- and boto3
# is not in the stack the first-run guide installs, so it has to be added to that
# environment before this roster can call anything.
REQUIRES = ("boto3",)
