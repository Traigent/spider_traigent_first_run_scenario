...
`2f744d1b`. For four days in September it was three lines that also said where to clone, beside
...
boundary in this bank -- a scorer it will not calibrate on the original -- and at `2f744d1b` the
...
guide revision `2f744d1b503ccdfec33f1be57d3e813cf042e863` (`2f744d1b`), on 2026-09-16.**
...
regeneration on the trunk that carries it (`e4096e3a`, 2026-09-15), re-taken at `2f744d1b` on
2026-09-16 after guide #551 bounded the first run to at most 28 rows from any source, #552
pinned SDK 0.27.0 and #553 marked the short-dataset top-up recommended: no score, band or action
moved, and the two cards whose action is `label-data` now read "Review the expected results this
...
`docs/measurements/cards/`. The checkout must be sitting on `2f744d1b`, and the run says so if
it is not.
...
committed cards, re-taken at `2f744d1b`.
...
max_trials; it reads 'knobs', 'source' and 'build'`, re-checked at `2f744d1b` -- and the
...
`references/run-safety.md`, and the wording quoted is the current one, `2f744d1b`.)
...
this the way it is written -- the quoted text against the guide at `2f744d1b`, and
...
`sql-exec-stop`'s, at `2f744d1b` as it was at `6ec2b9c1`.
...
Re-measured at `2f744d1b` ([`cards/grid-*`](docs/measurements/cards/)), the pair still pays:
...
checkout that is not sitting on the pinned revision, `2f744d1b`
