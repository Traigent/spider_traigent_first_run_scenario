...
> moved.** The score bank was regenerated at `9eaabbb2` on 2026-09-15 -- the current cards are
> under [`docs/measurements/cards/`](measurements/README.md) and the current table is in the
> repository README -- and at that revision every one of these projects opens at 45 under the
> agent ceiling, the run behind the 91 EXCELLENT number is refused by the calibration tool, and
> the declared pair below reads 45. The numbers in this section are not republished as
> current; they are the reading the guide's later changes answered.
...
probes shipped, read 86. At `9eaabbb2` both cards read 45, because the agent ceiling binds
first; the difference calibration makes is still in the cards' JSON records as `weighted_average`,
51 against 68, and the band above WORKABLE additionally waits for a row review. The 41-point
figure is a 6ec2b9c1 reading.
